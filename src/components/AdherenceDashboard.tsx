...
const plannedHHOf = (o: WorkOrder) => Math.max(0, Number(o.headcount) || 0) * Math.max(0, Number(o.estimatedHours) || 0);
...
export const AdherenceDashboard: React.FC<Props> = ({ workOrders }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const comparable = filtered.filter(o => plannedHHOf(o) > 0 && plannedHHOf(o) !== hhOf(o));
    return {
      volume, hh, days, volumeDay: days ? volume / days : 0, hhDay: days ? hh / days : 0,
      approved: filtered.filter(approved).length, evidence: filtered.filter(evidence).length,
      comparable: comparable.length, plannedHH: comparable.reduce((s, o) => s + plannedHHOf(o), 0),
      comparableHH: comparable.reduce((s, o) => s + hhOf(o), 0),
    };
  }, [filtered]);
  const series = useMemo(() => {
    const map = new Map<string, { key: string; label: string; volume: number; hh: number; orders: number }>();
    filtered.forEach(o => {
      if (!o.executionDate) return;
      const key = periodKey(o.executionDate, grain);
      const row = map.get(key) || { key, label: periodLabel(key, grain), volume: 0, hh: 0, orders: 0 };
      row.volume += volumeOf(o); row.hh += hhOf(o); row.orders += 1; map.set(key, row);
    });
    return [...map.values()].sort((a, b) => a.key.localeCompare(b.key));
  }, [filtered, grain]);
  const areas = useMemo(() => {
    const map = new Map<string, { name: string; volume: number; hh: number; orders: number }>();
    filtered.forEach(o => {
      const name = o.sectorName || o.areaName || 'Sin área informada';
      const row = map.get(name) || { name, volume: 0, hh: 0, orders: 0 };
      row.volume += volumeOf(o); row.hh += hhOf(o); row.orders += 1; map.set(name, row);
    });
  ...
  const maxV = Math.max(1, ...series.map(x => x.volume)); const maxHH = Math.max(1, ...series.map(x => x.hh));
  const maxAreaV = Math.max(1, ...areas.map(x => x.volume)); const maxAreaHH = Math.max(1, ...areas.map(x => x.hh));
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ...
      ['Indicador', 'Valor'], ['Volumen (m³)', metrics.volume], ['HH-persona', metrics.hh],
      ['Promedio m³/día activo', metrics.volumeDay], ['Promedio HH/día activo', metrics.hhDay],
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(series.map(x => ({ Período: x.label, 'Volumen (m³)': x.volume, 'HH-persona': x.hh, OT: x.orders }))), 'Serie temporal');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(areas.map(x => ({ 'Área operativa': x.name, 'Volumen (m³)': x.volume, 'HH-persona': x.hh, OT: x.orders }))), 'Por área');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filtered.map(o => ({
      ...
      Personas: o.headcount || 0, 'Duración (h)': o.realHours || 0, 'HH-persona': hhOf(o), 'Volumen (m³)': volumeOf(o),
  ...
  return <section className="kpi-dashboard">
    <header className="kpi-dashboard__header">
      ...
      <div className="kpi-dashboard__actions kpi-no-print"><button className="btn btn-secondary" onClick={() => window.print()}><Printer size={16} /> Imprimir / PDF</button><button className="btn btn-secondary" onClick={exportExcel}><FileSpreadsheet size={16} /> Exportar Excel</button></div>
    </header>
    ...
    {!filtered.length ? <div className="kpi-empty"><BarChart3 size={36} /><strong>No hay órdenes para los filtros seleccionados</strong><span>Amplía el período o restablece los filtros.</span></div> : <>
      <div className="kpi-analysis-grid">
        <article className="kpi-panel"><div className="kpi-panel__heading"><div><span>EVOLUCIÓN TEMPORAL</span><h3>m³ y HH por {grain === 'DAY' ? 'día' : grain === 'WEEK' ? 'semana' : 'mes'}</h3></div><small>{series.length} períodos</small></div><div className="kpi-series">
          {series.map(x => <div className="kpi-series__row" key={x.key}><div className="kpi-series__label"><strong>{x.label}</strong><span>{x.orders} OT</span></div><div className="kpi-series__tracks"><div className="kpi-track"><i className="kpi-track__bar kpi-track__bar--volume" style={{ width: `${Math.max(2, x.volume / maxV * 100)}%` }} /><b>{number(x.volume)} m³</b></div><div className="kpi-track"><i className="kpi-track__bar kpi-track__bar--hh" style={{ width: `${Math.max(2, x.hh / maxHH * 100)}%` }} /><b>{number(x.hh)} HH</b></div></div></div>)}
        </div><div className="kpi-legend"><span><i className="kpi-legend__volume" /> Volumen registrado</span><span><i className="kpi-legend__hh" /> Horas-hombre</span></div></article>
        <article className="kpi-panel"><div className="kpi-panel__heading"><div><span>DESGLOSE OPERACIONAL</span><h3>Producción por área</h3></div><small>{areas.length} áreas</small></div><div className="kpi-area-list">
          {areas.map(x => <div className="kpi-area-row" key={x.name}><div className="kpi-area-row__header"><strong title={x.name}>{x.name}</strong><span>{x.orders} OT</span></div><div className="kpi-area-row__metric"><span>m³</span><div><i style={{ width: `${Math.max(2, x.volume / maxAreaV * 100)}%` }} /></div><b>{number(x.volume)}</b></div><div className="kpi-area-row__metric kpi-area-row__metric--hh"><span>HH</span><div><i style={{ width: `${Math.max(2, x.hh / maxAreaHH * 100)}%` }} /></div><b>{number(x.hh)}</b></div></div>)}
        </div></article>
      </div>
      <div className="kpi-control-grid">
        ...
        <article className="kpi-panel kpi-panel--compact"><div className="kpi-panel__heading"><div><span>CALIDAD DEL REGISTRO</span><h3>Evidencias y planificación</h3></div><AlertTriangle size={19} /></div><div className="kpi-quality"><div><span>Evidencia antes + después</span><strong>{metrics.evidence}/{filtered.length}</strong><i><b style={{ width: `${metrics.evidence / filtered.length * 100}%` }} /></i></div><div><span>Aprobadas / completadas</span><strong>{metrics.approved}/{filtered.length}</strong><i><b style={{ width: `${metrics.approved / filtered.length * 100}%` }} /></i></div><div className="kpi-planning-note"><strong>{metrics.comparable ? `${metrics.comparable} OT comparables` : 'Plan vs. real: no evaluable'}</strong><span>{metrics.comparable ? `Plan ${number(metrics.plannedHH)} HH · Real ${number(metrics.comparableHH)} HH` : 'Las horas estimadas y reales están guardadas con el mismo valor. No se muestra una adherencia artificial de 100%.'}</span></div></div></article>
      </div>
    </>}
    <footer className="kpi-method-note"><strong>Metodología:</strong> HH = cantidad de personas × duración real de cada OT. Los promedios consideran sólo días con actividad. El volumen corresponde al m³ registrado o calculado en la OT; no se interpreta automáticamente como eficiencia.</footer>
  </section>;
};
